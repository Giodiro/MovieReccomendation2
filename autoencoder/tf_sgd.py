...
class MaskingNoiseAutoencoder:
    def __init__(self, name, n_input, n_hidden, dropout_prob,
        ...
        self.input_mean = tf.placeholder(tf.float32, [self.n_input], name="train_mean")
        self.input = tf.placeholder(tf.float32, [None, self.n_input], name="Input_ph")
        self.input_mask = tf.placeholder(tf.bool, [None, self.n_input], name="InputMask_ph") # True when input is invalid
        self.keep_prob = tf.placeholder(tf.float32, [], name="keep_probability")
        ...
        with tf.name_scope("noise"):
            self.noise_prob = tf.placeholder(tf.float32, [], name="gaussian_nois_prob")
            self.sap_prob = tf.placeholder(tf.float32, [], name="SAP_probability")
        ...
        with tf.name_scope("loss"):
            self.alpha = tf.placeholder(tf.float32, [], name="Alpha_ph")
            self.beta = tf.placeholder(tf.float32, [], name="Beta_ph")
            self.regl = tf.placeholder(tf.float32, [], name="Regl_ph")
            # Calculate loss
    ...
    def get_all_weights(self, sess):
        retw = {}
        for wkey in self.weights.keys():
            retw[wkey] = sess.run(self.weights[wkey])
        return retw
...
def main(sett, data_axis=0, make_predictions=False):
    ...
    model = MaskingNoiseAutoencoder(name="autoencoder",
